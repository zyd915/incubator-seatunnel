...
public class ClickHouseDynamicTableFactory
    ...
    private void validateConfigOptions(ReadableConfig config) {
        ...
        } else if (config.getOptional(SCAN_PARTITION_COLUMN).isPresent() ^ config.getOptional(SCAN_PARTITION_NUM).isPresent() ^ config.getOptional(SCAN_PARTITION_LOWER_BOUND).isPresent() ^ config.getOptional(SCAN_PARTITION_UPPER_BOUND).isPresent()) {
            throw new IllegalArgumentException(
