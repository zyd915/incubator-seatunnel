...
public class ClickHouseRowConverter implements Serializable {
    ...
    public static final long SECOND_1000 = 1_000_000L;
    ...
    protected ClickHouseRowConverter.DeserializationConverter createToInternalConverter(
        ...
        switch (type.getTypeRoot()) {
            ...
            case DECIMAL:
                ...
                return val ->
                        val instanceof BigInteger ? DecimalData.fromBigDecimal(new BigDecimal((BigInteger) val, 0), precision, scale)
                                : DecimalData.fromBigDecimal((BigDecimal) val, precision, scale);
            case DATE:
                return val -> (int) ((Date) val).toLocalDate().toEpochDay();
            case TIME_WITHOUT_TIME_ZONE:
                return val -> (int) (((Time) val).toLocalTime().toNanoOfDay() / SECOND_1000);
    ...
    protected ClickHouseRowConverter.SerializationConverter createToExternalConverter(
        ...
        switch (type.getTypeRoot()) {
            ...
            case TIME_WITHOUT_TIME_ZONE:
                return (val, index, statement) -> {
                    LocalTime localTime = LocalTime.ofNanoOfDay(val.getInt(index) * SECOND_1000);
            ...
            case MAP:
                return (val, index, statement) ->
                        statement.setObject(index + 1, ClickHouseConverterUtils.toExternal(val.getMap(index), type));
            case MULTISET:
            case ROW:
            case RAW:
            default:
